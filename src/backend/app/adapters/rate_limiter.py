"""토큰버킷 레이트 리미터 — 벤더 RPM/TPM(분당 요청·토큰) 준수용 프로액티브 큐 제어.

동기 스레드풀 환경에서 LLM 호출을 페이싱한다(429를 사후 재시도만 하지 않고, 미리 속도를 제어).
`acquire(cost)`는 버킷에 cost만큼 여유가 있으면 즉시 True, 없으면 대기하되:
- 대기 예상이 max_wait를 넘으면 False(→ 호출측이 429로 매핑),
- 이미 대기 중인 요청이 max_waiters면 즉시 False(스레드풀 고갈·무한대기 방지),
- cost가 버킷 용량(capacity)보다 크면 영원히 채워지지 않으므로 즉시 False.
cost를 항상 1로 쓰면 RPM(요청 개수) 리미터, 요청마다 추정 토큰량을 넘기면 TPM(토큰량) 리미터가 된다
(groq_adapter.py가 후자로 사용).
"""
...
class TokenBucketLimiter:
    """분당 rate_per_min만큼 충전되는 토큰버킷(burst만큼 순간 허용)."""
    ...
    def __init__(
        ...
        self._rate = max(1e-4, rate_per_min / 60.0)  # 초당 충전량
    ...
    def acquire(self, cost: float = 1.0) -> bool:
        ...
        if cost > self._capacity:
            return False  # 아무리 기다려도 이 버킷 용량으로는 충족 불가
        with self._lock:
            self._refill_locked()
            ...
            while True:
                with self._lock:
                    ...
                    if self._tokens >= cost:
                        self._tokens -= cost
                        return True
                    need = (cost - self._tokens) / self._rate
