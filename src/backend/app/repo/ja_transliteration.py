...
_LOANWORD_HANGUL: dict[str, str] = {
    ...
    "びっぐまうす": "빅마우스",  # ビッグマウス (사용자 확인 — 촉음 규칙만으론 "빅구마우스"가 되어 어긋남)
    "れしぴ": "레시피",  # レシピ
    "るふらん": "루프랑",  # ルフラン
    "るからか": "루카루카",  # ルカルカ
    "るうぷ": "루프",  # ループ
...
_JONG_GIYEOK = _JONG.index("ㄱ")
_JONG_BIEUP = _JONG.index("ㅂ")

# 촉음(っ) 뒤 자음의 조음 위치에 따른 받침 근사(외래어에서 흔함 — ビッグ→"빅", トップ→"탑").
# 원래 촉음은 다음 자음을 겹치는(게미네이션) 부호라, 받침은 항상 ㅅ이 아니라 그 다음
# 자음 계열을 따라야 실제 발음/관용 표기에 가깝다. か/が행은 ㄱ, ぱ/ば행은 ㅂ, 나머지(さ/ざ/
# た/だ행 등)는 기존 관용대로 ㅅ을 유지한다.
_SOKUON_GIYEOK_ROW = set("かきくけこがぎぐげご")
_SOKUON_BIEUP_ROW = set("ぱぴぷぺぽばびぶべぼ")


def _sokuon_jong(nxt: str) -> int:
    if nxt in _SOKUON_GIYEOK_ROW:
        return _JONG_GIYEOK
    if nxt in _SOKUON_BIEUP_ROW:
        return _JONG_BIEUP
    return _JONG_SIOT
...
def kana_to_hangul(hira: str) -> str:
    ...
    - 촉음(っ): 앞 음절에 받침을 얹는다(있으면 덮어쓰지 않음) — 뒤따르는 자음이 か/が행이면
      ㄱ받침(ビッグ→"빅"), ぱ/ば행이면 ㅂ받침(トップ→"탑"), 그 외(さ/ざ/た/だ행 등)는 ㅅ받침.
