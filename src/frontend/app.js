...
const MIN_SEGMENTS = 2;               // 최소 구간 수(= 분리선 1개)
const MAX_SEGMENTS = 11;              // 최대 구간 수(= 분리선 10개, 참조 문서 사양)
const MAX_BOUNDARIES = MAX_SEGMENTS - 1;
const DEFAULT_SEGMENTS = 3;           // 초기 구간 수(응답 전 표시용)

let stageModel = null; // { totalMinutes, segments: [{energy(0~1), width(합=1)}] }
let isDragging = false; // 점/경계 드래그 중 — 드래그 중엔 우클릭/롱프레스 메뉴를 열지 않음
...
function initStageModel() {
  // 에너지 단계 수는 이제 그래프(우클릭/롱프레스 추가·제거)로만 조정 — 초기값은 기본 구간 수.
  const n = DEFAULT_SEGMENTS;
...
function renderStageGraph() {
  ...
  hint.textContent = "● 위·아래 = 에너지  ·  ◆ 좌·우 = 구간 길이  ·  우클릭(모바일: 길게 누르기) = 단계 추가·제거";
  ...
  wireGraphMenu(plot); // 우클릭(데스크톱) / 1초 롱프레스(모바일) → 단계 추가·제거 메뉴
...
function bindDrag(node, graph, onMove) {
  node.addEventListener("pointerdown", (e) => {
    ...
    const move = (ev) => {
      isDragging = true;   // 실제 이동 발생 → 이 제스처는 드래그(메뉴 억제)
...
// ── 에너지 단계 추가·제거 메뉴 (사용자 참조 문서 hofix_comment/핫픽스 제안 2) ──────────────────
// 데스크톱: 우클릭 / 모바일: 1초 롱프레스. 드래그 중에는 열리지 않는다(isDragging + 이동 임계값).
// 메뉴는 항상 두 항목([단계 추가]/[단계 제거])을 표시한다(사용자 선택).
function wireGraphMenu(plot) {
  // 데스크톱 우클릭
  plot.addEventListener("contextmenu", (e) => {
    e.preventDefault();
    if (isDragging) return;
    const r = plot.getBoundingClientRect();
    openStageMenu(e.clientX, e.clientY, clamp01((e.clientX - r.left) / r.width), false);
  });

  // 모바일 1초 롱프레스(마우스는 contextmenu가 담당). 이동/떼기/취소 시 타이머 해제.
  let timer = null, sx = 0, sy = 0, sfx = 0;
  const cancel = () => { if (timer) { clearTimeout(timer); timer = null; } };
  plot.addEventListener("pointerdown", (e) => {
    if (e.pointerType === "mouse") return;
    sx = e.clientX; sy = e.clientY;
    const r = plot.getBoundingClientRect();
    sfx = clamp01((e.clientX - r.left) / r.width);
    cancel();
    timer = setTimeout(() => {
      timer = null;
      if (isDragging) return;      // 드래그 중이면 열지 않음
      openStageMenu(sx, sy, sfx, true);
    }, 1000);
  });
  plot.addEventListener("pointermove", (e) => {
    if (timer && Math.hypot(e.clientX - sx, e.clientY - sy) > 10) cancel(); // 이동=드래그/스크롤 의도
  });
  plot.addEventListener("pointerup", cancel);
  plot.addEventListener("pointercancel", cancel);
}

let stageMenuEl = null;
function onStageMenuOutside(e) { if (stageMenuEl && !stageMenuEl.contains(e.target)) closeStageMenu(); }
function onStageMenuEsc(e) { if (e.key === "Escape") closeStageMenu(); }
function closeStageMenu() {
  if (!stageMenuEl) return;
  stageMenuEl.remove();
  stageMenuEl = null;
  document.removeEventListener("pointerdown", onStageMenuOutside, true);
  document.removeEventListener("keydown", onStageMenuEsc, true);
}

function openStageMenu(clientX, clientY, fx, viaTouch) {
  closeStageMenu();
  if (!stageModel) return;
  const n = stageModel.segments.length;
  const menu = elDiv("stage-menu");

  const addBtn = document.createElement("button");
  addBtn.type = "button";
  addBtn.textContent = "＋ 여기에 에너지 단계 추가";
  addBtn.addEventListener("click", () => {
    if (stageModel.segments.length >= MAX_SEGMENTS) {
      showGraphToast(`구간 분리선은 최대 ${MAX_BOUNDARIES}개까지 만들 수 있어요!`);
    } else {
      addStageAt(fx);
    }
    closeStageMenu();
  ...
  delBtn.type = "button";
  delBtn.textContent = "－ 이 에너지 단계 제거";
  if (n <= MIN_SEGMENTS) {
    delBtn.disabled = true;
    delBtn.title = `최소 ${MIN_SEGMENTS}개 구간은 있어야 해요`;
  } else {
    delBtn.addEventListener("click", () => { removeStageNear(fx); closeStageMenu(); });
  }

  menu.append(addBtn, delBtn);
  document.body.appendChild(menu);

  // 뷰포트 밖으로 넘치지 않게 위치 보정.
  ...
  // 터치 롱프레스: 손가락을 뗄 때 발생하는 유령 클릭이 메뉴 버튼을 즉시 누르는 것을 막는다
  // (여는 순간 손가락이 메뉴 위에 있으므로). 첫 클릭 1회만 삼킨다.
  if (viaTouch) {
    const openedAt = Date.now();
    const swallow = (ev) => {
      document.removeEventListener("click", swallow, true);
      if (Date.now() - openedAt < 700 && stageMenuEl && stageMenuEl.contains(ev.target)) {
        ev.stopPropagation();
        ev.preventDefault();
      }
    };
  ...
  // 여는 제스처의 이벤트로 즉시 닫히지 않도록 다음 틱에 바깥 클릭 리스너 등록.
  setTimeout(() => {
    document.addEventListener("pointerdown", onStageMenuOutside, true);
    document.addEventListener("keydown", onStageMenuEsc, true);
  }, 0);
}

// 추가: fx가 속한 구간을 그 지점에서 둘로 분할. 새 구간(오른쪽) 에너지 = 앞·뒤 값의 평균.
function addStageAt(fx) {
  const segs = stageModel.segments;
  if (segs.length >= MAX_SEGMENTS) return;
  const cum = [0];
  segs.forEach((s) => cum.push(cum[cum.length - 1] + s.width));
  let k = segs.length - 1;
  for (let i = 0; i < segs.length; i++) { if (fx < cum[i + 1]) { k = i; break; } }
  const w = segs[k].width;
  let localFx = fx - cum[k];
  localFx = w >= 2 * MIN_WIDTH ? clamp(localFx, MIN_WIDTH, w - MIN_WIDTH) : w / 2;
  const eK = segs[k].energy;
  const eNext = k + 1 < segs.length ? segs[k + 1].energy : eK;
  const newEnergy = clamp01(+((eK + eNext) / 2).toFixed(2));
  segs.splice(k, 1, { energy: eK, width: localFx }, { energy: newEnergy, width: w - localFx });
  stageTouched = true;
  renderStageGraph();
}

// 제거: 클릭 좌표에서 가장 가까운 에너지 포인트의 오른쪽 분리선을 없앤다(마지막 포인트면 왼쪽).
// 병합 구간은 왼쪽(앞) 에너지를 유지하고 폭을 합산 — 마지막 제거 시 직전 구간이 끝까지 대신한다.
function removeStageNear(fx) {
  const segs = stageModel.segments;
  ...
  centers.forEach((c, idx) => { const d = Math.abs(c - fx); if (d < best) { best = d; i = idx; } });
  const b = i < segs.length - 1 ? i : segs.length - 2; // 제거할 분리선(구간 b와 b+1 사이)
  segs.splice(b, 2, { energy: segs[b].energy, width: segs[b].width + segs[b + 1].width });
  stageTouched = true;
  renderStageGraph();
}
...
function showGraphToast(msg) {
  let t = $("graph-toast");
  if (!t) { t = elDiv("graph-toast"); t.id = "graph-toast"; document.body.appendChild(t); }
  t.textContent = msg;
  t.classList.add("show");
  if (graphToastTimer) clearTimeout(graphToastTimer);
  graphToastTimer = setTimeout(() => t.classList.remove("show"), 2200);
}
...
function reflectSettings(data) {
  ...
  // 단계 수는 텍스트박스 대신 에너지 그래프가 표현 — syncGraphToParams가 아크로 반영한다.
  if (!coverTouched) {
