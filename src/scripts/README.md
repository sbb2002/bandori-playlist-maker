...
- 데이터 반영은 `git-rules.md`의 `data` 단일 브랜치에 자동 커밋·푸시한다(기본 동작, PR 없음).
  **`data`는 `main`에 병합되지 않는 독립 브랜치다**(2026-07-15 확정) — 배포된 backend가
  런타임에 `data` 브랜치를 직접 원격 fetch하므로(`src/backend/app/repo/remote_source.py`)
  main 병합 자체가 애초에 불필요하다.
