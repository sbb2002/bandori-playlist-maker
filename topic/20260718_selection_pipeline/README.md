...
```bash
# Option 1: File
mkdir -p topic/20260718_selection_pipeline/work
echo "your-api-key-here" > topic/20260718_selection_pipeline/work/groq.key
...
### 선행 단계 완료
- `topic/20260720_vector_embedding/src/method-1/` ✓ Stage 1 완료
  - `full_catalog_songs.csv` (661곡)
  - `out/song_profiles.csv` (가사 기반 프로파일)
- `topic/20260720_vector_embedding/src/method-2/` ✓ Stage 2 완료
...
C:/Users/User/miniconda3/envs/warmth/python.exe topic/20260718_selection_pipeline/01_query_targets.py
```

**산출물**: `topic/20260718_selection_pipeline/out/query_targets.csv`
...
```bash
$env:PYTHONIOENCODING = "utf-8"
C:/Users/User/miniconda3/envs/warmth/python.exe topic/20260718_selection_pipeline/02_band_percentiles.py
```

**산출물**: `topic/20260718_selection_pipeline/out/song_band_percentiles.csv`
...
```bash
$env:PYTHONIOENCODING = "utf-8"
C:/Users/User/miniconda3/envs/warmth/python.exe topic/20260718_selection_pipeline/03_lyric_candidates.py
```

**산출물**: 
- `topic/20260718_selection_pipeline/out/queries_expanded.csv` (LLM 확장 결과)
- `topic/20260718_selection_pipeline/out/query_lyric_candidates.csv` (각 쿼리별 top-N)
...
```bash
$env:PYTHONIOENCODING = "utf-8"
C:/Users/User/miniconda3/envs/warmth/python.exe topic/20260718_selection_pipeline/04_run_arms.py
```

**산출물**: `topic/20260718_selection_pipeline/out/method3_arm_results.csv`
...
```bash
$env:PYTHONIOENCODING = "utf-8"
C:/Users/User/miniconda3/envs/warmth/python.exe topic/20260718_selection_pipeline/05_build_blind_sheet.py
```

**산출물**:
1. **`topic/20260718_selection_pipeline/out/method3_blind_sheet.csv`** (평가용)
...
2. **`topic/20260718_selection_pipeline/out/method3_blind_mapping.csv`** (언블라인드용, 평가 전까지 미열기)
...
```python
# Python에서 언블라인드
df_blind = pd.read_csv("topic/20260718_selection_pipeline/out/method3_blind_sheet.csv")
df_mapping = pd.read_csv("topic/20260718_selection_pipeline/out/method3_blind_mapping.csv")
...
```bash
# PowerShell
Get-Content -Raw "topic/20260718_selection_pipeline/out/01_query_targets_progress.json" | ConvertFrom-Json
```
...
```
topic/20260718_selection_pipeline/
├── DESIGN.md                          # 설계 문서 (필독)
├── README.md                          # 이 파일
...
- `topic/20260720_vector_embedding/report/05-energy_distribution_by_band.md` — 밴드별 강도 분포 (band_pct 맥락)
- `topic/20260720_vector_embedding/src/method-1/06_stage2_search.py` — 가사 임베딩 검색 (재사용 참고)
- `topic/20260720_vector_embedding/src/method-2/DESIGN.md` — 음향 지표 정의 (intensity 축)
