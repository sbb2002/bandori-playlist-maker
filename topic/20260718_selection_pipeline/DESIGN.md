...
> **위치와 성격**: `topic/20260720_vector_embedding`의 하위 method가 아니라 **별개 주제**다. 가사 임베딩은
...
> 저장소 루트 기준 전체경로로 표기한다(예: `topic/20260720_vector_embedding/src/method-2/...`).
>
> **배경**: `topic/20260720_vector_embedding/report/05-energy_distribution_by_band.md`가 프로덕션
...
>
> **Phase 2(`topic/20260720_vector_embedding/src/method-2`)와의 차이 — 중요**: Phase 2의 결합(arm C)은
> 가사 백분위와 음향 백분위를 **가중합(α=0.5)**해 하나의 점수로 섞었고, 이 복합 점수(`acou_match`)가
> 만족도와 ρ=−0.588로 **유해**했다(`topic/20260720_vector_embedding/report/04-lyrics_acoustic_fusion.md`
...
| 밴드 미지정 + 전체카탈로그 절대 표현("진짜 조용한 노래") | **arm1·arm2 ≥ arm3** — 상대 백분위를 여러 밴드에 걸쳐 쓰면 "조용함"의 절대적 의미가 깨짐(`topic/20260720_vector_embedding/report/05-energy_distribution_by_band.md` 트레이드오프) |
...
- LLM이 자연어 쿼리에서 **`intensity_target`(0~1, 661곡 백분위 목표)** 와 **`band_filter`(밴드
  태그 1개 또는 NA)** 를 추출한다. 프롬프트는 `topic/20260720_vector_embedding/src/method-2/DESIGN.md` §3의
  `INTENSITY` 축 정의를 그대로 승계(음량/밀도, 감정 아님을 명시)하고, `BAND`는 카탈로그 밴드 태그
  11개 중 하나 또는 `NA`로 강제한다. 모델은 `topic/20260720_vector_embedding/src/method-1/config.py`의
  `GROQ_MODEL`(현재 `meta-llama/llama-4-scout-17b-16e-instruct`) — **`openai/gpt-oss-20b`
  절대 금지**(배포 모델).
- `eligible_pool` = `topic/20260720_vector_embedding/src/method-1/full_catalog_songs.csv` 661곡 중
  `band_filter`가 있으면 그 밴드만(프로덕션 `build_setlist`의 band_filter 적용 순서와 동일 —
  후보추림보다 먼저 적용).
- `intensity`/`intensity_pct`는 `topic/20260720_vector_embedding/src/method-2/out/song_acoustics.csv`를
  그대로 재사용(재계산 금지 — 이미 검증된 값, `topic/20260720_vector_embedding/src/method-2/DESIGN.md` §2).
- `band_pct(곡)` = 그 곡의 **자기 밴드 population**(`eligible_band==True`인 동일 밴드 전체) 내
  `intensity` 백분위. `topic/20260720_vector_embedding/report/05-energy_distribution_by_band.md`의 표
...
밴드 태그는 `topic/20260720_vector_embedding/src/method-1/full_catalog_songs.csv`의 실제 값(`mygo`,
...
`topic/20260720_vector_embedding/src/method-1/06_stage2_search.py`의 방식을 **그대로 재사용**한다 —
새로 설계하지 않는다:
1. LLM으로 쿼리를 2~3문장 서술로 확장(`06_stage2_search.py`의 `expand_prompt` 템플릿 그대로).
2. `BAAI/bge-m3`로 확장문과 `topic/20260720_vector_embedding/src/method-1/out/song_profiles.csv`의
   `desc`를 임베딩, 코사인 유사도.
...
`topic/20260718_selection_pipeline/` 아래:
...
연구자 청취·채점 (1~5)
  ↓
언블라인드 → §0 판정표 적용 → topic/20260718_selection_pipeline/report/01-selection_pipeline_comparison.md
```
