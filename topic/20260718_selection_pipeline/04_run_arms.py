"""
Step 4: Run three arms and extract top-K results per query.

DESIGN.md §1: Arm 1 (production), Arm 2 (lyrics+intensity), Arm 3 (lyrics+band_pct).
Each arm produces top-K=3 results per query.

Output: out/method3_arm_results.csv
...
def load_data():
    ...
    df_candidates = pd.read_csv(config.OUT_DIR / "query_lyric_candidates.csv")
...
def run_arm3(candidate_pool, intensity_target):
    """
    Arm 3: Lyrics screening + band-relative percentile.

    Algorithm: Same as Arm 2 but using band_pct instead of intensity.
    band_pct is pre-computed and never re-percentiled within candidate_pool.
    """
    if pd.isna(intensity_target):
        # No intensity target -> return top of pool by lyric cosine
        return candidate_pool.head(config.K)

    # Create window using band_pct
    window = candidate_pool[
        np.abs(candidate_pool["band_pct"] - intensity_target) <= config.TOL
    ]

    if len(window) >= config.K:
        # Window has enough songs -> sort by band_pct proximity
        window = window.copy()
        window["_dist_to_target"] = np.abs(window["band_pct"] - intensity_target)
        candidates = window.sort_values("_dist_to_target")
    else:
        # Fallback to all in pool, sorted by band_pct proximity
        candidates = candidate_pool.copy()
        candidates["_dist_to_target"] = np.abs(candidates["band_pct"] - intensity_target)
        candidates = candidates.sort_values("_dist_to_target")
...
def main():
    ...
    output_csv = config.OUT_DIR / "method3_arm_results.csv"
    ...
    for _, target_row in df_targets.iterrows():
        ...
        # Arm 2/3: candidate_pool = eligible songs that are lyric candidates
        ...
        # Arm 3: Lyrics + band_pct
        if len(df_candidates_for_query) > 0:
            arm3_results = run_arm3(df_candidates_for_query.copy(), intensity_target)
            for rank, (_, row) in enumerate(arm3_results.iterrows(), 1):
                results.append({
                    "query_id": query_id,
                    "arm": "arm3",
                    "rank": rank,
            ...
            print(f"    Arm3 top-{min(3, len(arm3_results))}: {arm3_results['tag'].tolist()}")
        else:
            print(f"    Arm3 skipped (no candidates)")
