...
> 설계: `topic/20260718_selection_pipeline/DESIGN.md`(사전 등록). 블라인드 채점(1~5 Likert, `random.Random(20260717)`
...
이는 `topic/20260720_vector_embedding/report/04-lyrics_acoustic_fusion.md`·[[vector-embedding-phase2-status]]가
...
3. **인프라 설계** (arm2를 어느 범위로 하든 공통으로 필요):
   - `desc`(가사 요약) 필드를 661곡 전체에 대해 오프라인 사전계산 — 소스는
     `topic/20260720_vector_embedding/src/method-1/out/song_profiles.csv`의 기존 산출물을 재사용 가능한지
...
> 원본 분석: `topic/20260718_selection_pipeline/note/stat_arm1vs2.md`(재현 절차 포함). §0·§6은 arm1(4.333)
