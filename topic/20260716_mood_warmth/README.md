...
### 청취 검증 결과 (2026-07-15, f0_range 극단 6곡)
- 상위 3곡: BLACK SHOUT=극적 · QUEEN=약간 극적 · in your blue eyes=**애절** (3/3 적중)
- 하위 3곡: V.I.P=애매 · COLORFUL BOX=애매(애절 절대 아님) · Steer to Utopia=약간 극적 (2/3 적중, 경미한 반례 1)
- **f0_range = "극적임" 축으로 확증. 단 극적↔애절을 가르지 못함** — 앵커 분석과 일치.

### 사후 탐색 (post-hoc — 사전등록 아님, 차기 라운드에서 재검증 필요)
청취평의 극적/애절 구분을 발성 데이터와 대조한 결과:
- 애절/가련 평가곡(in your blue eyes, Georgette)은 **전체 최저 shimmer·최고 HNR**(가장 맑은 발성),
  극적 평가곡(BLACK SHOUT, QUEEN)은 정반대(최고 jitter/shimmer·최저 HNR).
- → **애절함 = 넓은 F0 음역 × 맑은 발성** 조합 가설. 합성지표
  `c3 = z(f0_range)+z(HNR)−z(shimmer)`가 곡 단위에서 완벽 분리:
  in your blue eyes 97pct · Georgette/esora/Sonorous/天球 93pct vs BLACK SHOUT 24pct.
- **그러나 c3는 현 rating과 무상관(ρ=−0.086)** — rating("esora 유사도")이 밝기 혼재 축이라
  애절함 검증 도구로 부적합함이 확정됨. H1(jitter/shimmer/HNR)은 "기각"이 아니라
  **"방향 반전된 조합 성분으로 부활"**(거친 절규 아닌 맑은 고음이 애절함의 원천) — 단 이 해석은
  알려진 6곡+앵커에 대한 사후 관찰이므로 독립 라벨로 재검증해야 함.
...
- 검증 완료 시 보고서 .md는 `document-archive`의 `archive/last-papers/research/`로 별도 커밋.
