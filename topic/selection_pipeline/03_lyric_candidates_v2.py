...
def expand_queries(client, progress):
    ...
    Cache the results.
    """
    targets_csv = config.OUT_DIR / "query_targets_v2.csv"
    df_targets = pd.read_csv(targets_csv)

    queries_expanded_csv = config.OUT_DIR / "queries_expanded_v2.csv"
    existing = set()
    results = []
...
def embed_and_search(df_queries_expanded, df_targets, progress):
    ...
    candidates_csv = config.OUT_DIR / "query_lyric_candidates_v2.csv"
    existing = set()
    results = []
...
def main():
    ...
    if not targets_csv.exists():
        print(f"ERROR: {targets_csv} not found. Run 01_query_targets_v2.py first.")
        sys.exit(1)
    df_targets = pd.read_csv(targets_csv)
