...
- **곡 데이터**: `data/songs_master.csv`(661곡, `song_repo.load_songs()` 그대로 사용) — v1·v2가
  썼던 `topic/vector_embedding`의 `song_acoustics.csv` intensity가 아니라, **energy_full +
  acousticness_proxy + 시간분절 강도(i_min/i_mean/i_end)의 soft-OR 결합**(실제 검증된 값)을 쓴다.
- **가사 요약(Stage C용 `desc`)**: `topic/vector_embedding/src/method-1/out/song_profiles.csv`의
