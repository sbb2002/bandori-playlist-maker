...
`topic/20260720_audio_feats_analysis/src/method-1/config.py`의 `KS_PROFILES` 생성부:
...
`topic/20260720_audio_feats_analysis/src/method-1/config.py`:
```python
...
등 발췌 기반 지표와 같은 파일에 들어있음 — `topic/20260720_vector_embedding/report/02` §3 참조).
...
1. **661곡 전체 재산출**: `topic/20260720_audio_feats_analysis/src/method-1/extract_features.py`를
...
4. `mode_score`(연속값) 자체는 이번 버그의 직접 영향권 밖일 가능성이 있다 —
   `topic/20260720_vector_embedding/report/02` §2b가 별도 ground-truth(65행 라벨)로 방향성을
   검증했고 그때도 이 버그는 존재했었다. 다만 그 검증은 "장/단조 분리가 방향은 맞다"까지만
   확인했지 "개별 곡 라벨이 정확하다"는 아니었으므로, 재산출 후 같은 ground-truth로 재검증
   권장.

## 8. 산출물
- 코드 수정: `topic/20260720_audio_feats_analysis/src/method-1/config.py` (`KS_PROFILES` 생성부,
  `np.roll` 부호 수정)
