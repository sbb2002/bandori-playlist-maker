...
- 결과를 `topic/20260720_audio_feats_analysis/out/essentia_key.csv`에 저장, `audio_feats.csv`의
...
`topic/20260720_audio_feats_analysis/out/key_far_disagreement_50.csv`(`mode_score` 절대값 오름차순
...
3. **far 불일치 50곡만 사람이 직접 청취 확인** — `key_far_disagreement_50.csv` 순서대로
   진행 권장. 이 리스트는 chord_progression 리서치의 "청취 스팟체크" 인계(`topic/20260718_chord_progression/HANDOFF.md`)
...
## 6. 산출물
- `topic/20260720_audio_feats_analysis/out/essentia_key.csv` (661행, Essentia 원본 결과)
- `topic/20260720_audio_feats_analysis/out/key_agreement_full.csv` (661행, est_key·essentia_key 병합 +
  일치 여부)
- `topic/20260720_audio_feats_analysis/out/key_far_disagreement_50.csv` (사람 청취 확인 대상 50곡,
  `mode_score` 절대값 오름차순)
