...
곡 pool은 `topic/20260716_mfcc_analysis`의 30곡 세트(10밴드×3곡)를 재사용한다. 보컬 스템(demucs)이
...
1. `topic/20260716_mfcc_analysis/stems/htdemucs/<tag>/vocals.wav`가 30곡 모두 존재해야 한다
   (`<tag>`는 `selected_songs.csv`의 `tag` 컬럼, 예: `afterglow__000`).
   **없으면 이 파이프라인에서 demucs를 다시 구현하지 말고**, `topic/20260716_mfcc_analysis/README.md`의
...
- **`00_prepare_stems.py` 추가**: `SAMPLE_TAGS` 중 `STEMS_DIR/<tag>/vocals.wav`가 없는 곡만
  ...
  단, 분리 로직을 새로 짜지 말고 `topic/20260716_mfcc_analysis/_demucs_run.py`의 몽키패치 러너를
...
- 2026-07-17 전체 카탈로그(661곡) ASR 확장 중 발견: faster-whisper는 언어를 파일당 한 번만
  감지해 전체 트랙에 고정 적용한다(세그먼트별 재감지 없음). raise_a_suilen(일본어+영어 혼용이
  타 밴드보다 잦음)·ave_mujica(일본어+영어+라틴어 계통 혼용, 예: ave_mujica__081 "Choir 'S'
  Choir")처럼 한 곡에 언어가 섞이면 비고정 언어 구간이 강제로 잘못 디코딩되거나(예: "satanic"
  70회 반복 환각) 신뢰도(`avg_logprob_mean`)가 떨어지는 것으로 추정 — 두 밴드가 신뢰도 하위
  15곡 중 다수를 차지함(`out/transcripts_meta.csv`로 확인 가능). **의도적으로 전처리하지
  않기로 결정**: 영향 곡이 661곡 중 약 20곡 안팎으로 적고, 세그먼트별 언어 재감지 전처리
  비용 대비 Stage 2 실사용 성능 개선이 크지 않을 것으로 판단(구현 난이도 대비 저효용). 원인
  분석은 완료했으므로 후속 세션이 재검토할 근거로 여기 기록만 남긴다.
  mugendai_mutype(보컬이 빠른 편이라는 가설)은 확인 결과 신뢰도가 카탈로그 중앙값과
  거의 동일(-0.261 vs -0.258)해 별도 이슈 없음으로 결론.
