...
**설계 재검토 트리거**: 1차 통과 피쳐 중 **과반수**가 위 기준으로 "밴드 의존적"으로
플래그되면, 현재 비례배분 설계가 장르 편중 문제를 실제로 일으킨 것으로 간주하고
**밴드당 동일 N(현재 총 70곡 기준 밴드당 7곡)으로 표본을 다시 뽑아 확증 라운드를
진행한다.** 과반수 미만이면(개별 피쳐 몇 개만 플래그) 현재 설계를 유지하고, 플래그된
피쳐는 보고서에 "밴드 구성에 민감함" 한계로만 명시한다(§6의 밴드 기여도 태깅과 동일한
정신 — 배제가 아니라 표시).
