...
- **피쳐 신뢰도 등급(2026-07-22 추가)** — 불신뢰 피쳐를 그대로 스크리닝에 섞으면 실제 신호가
  노이즈에 묻혀 "예측 불가"로 오판(false negative)하거나, 우연한 잡음이 유의해 보이는 오판
  (false positive)이 나올 수 있다. `audio_feats_analysis/report/06` §5(`report/09·10`으로
  갱신)를 근거로 3등급으로 나눈다:

  | 등급 | 피쳐 | 근거 |
  |---|---|---|
  | **Tier 1(신뢰)** | `mfcc_*`, `contrast`, `energy_full`, `rms`/`rms_mean`, `final_bpm`(bestdori 공식 출처, 661곡 중 573곡) | 반복 검증, 불신뢰 판정 이력 없음 |
  | **Tier 2(조건부·주의)** | `mode_score`(방향 검증됐으나 `key`와 계산 계보 공유 — roll 버그 수정 후 재검증 여부 불확실), `drum_tempo_bpm`(bestdori 미보유 88곡만 해당, method-2로 개선했으나 옥타브 오차 잔존) | 유효 가능성 있으나 단독 근거로는 약함 |
  | **Tier 3(불신뢰, 스크리닝 제외)** | `key`/`est_key`/`camelot`(버그 수정 후에도 Essentia와 55.4%만 일치, 50곡 원인불명 잔존), `energy`/`energy_proxy`/`acousticness_proxy`/`instrumentalness_proxy`(부호역전 확정), 코드진행 케이던스 파생값(정답률 악화로 폐기) | `report/06` §5, `report/08`, `report/10` |

  **적용 규칙**: Tier 1만 1차 판정 근거로 인정한다. Tier 2는 돌려보되 결과에 "저신뢰 피쳐
  기반, 단독 확증 아님" 표시를 하고 Tier 1이 이미 통과시킨 항목의 보조 근거로만 쓴다.
  **Tier 3은 스크리닝 자체에서 제외** — 우연히 상관이 나와도 "통과"로 인정하지 않는다.
