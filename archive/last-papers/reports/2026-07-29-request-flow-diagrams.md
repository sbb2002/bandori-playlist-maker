...
**엔드포인트별 실제 호출 시점** — `src/frontend/app.js` 기준. 셋 다 "세트리스트 생성"과는
무관한 UI 부트스트랩용이며, 공통 패턴은 **한 번 가져와서 캐시해두고 재사용**한다(요청마다
다시 부르지 않음).

| 엔드포인트 | 트리거 | 빈도 | 근거 |
|---|---|---|---|
| `/api/bands` | 페이지 로드 즉시 | 세션당 1회 | `app.js:618 loadBands()` — 스크립트 최상단에서 다른 초기화와 함께 즉시 실행. 밴드 필터 체크박스(밴드명+곡 수) 렌더링용. 이후 재호출 없음 |
| `/api/health` | 페이지 로드 시, **빌드 버전 주입이 없을 때만** | 조건부 1회(폴백) | `app.js:625-642 initVersion()` — 배포 프론트는 빌드 시 `window.APP_VERSION`/`APP_MAIN_VERSION`을 정적 주입해두므로 그 값이 있으면 백엔드를 아예 안 부름. 로컬 개발처럼 빌드 주입이 없을 때만 `version` 필드로 대체(메뉴의 버전 표기용) — "매번 켜지는 경로"가 아니라 주입 실패 시 폴백 |
| `/api/songs` | **"+ 곡 추가" 버튼 최초 클릭** (지연 로딩) | 세션당 최대 1회 | `app.js:1663-1668 ensureSongs()` / `1671 openSongPickerAt()` — 페이지 로드 때는 안 부르고, 사용자가 세트리스트에 곡을 수동 삽입하려고 모달을 처음 열 때만 호출. `allSongs` 변수에 캐시(`if (allSongs) return allSongs`)해 이후 재오픈 시 재호출 없음. 660곡 전체 목록이라 필요할 때만 가져오는 방식 |
...
2. HTTP 호출이 실패(네트워크·Render 일시 장애·토큰 불일치·타임아웃) → `except`로 삼키고
   경고만 남김.
3. ~~`DATA_REFRESH_TOKEN`이 Render 쪽에 아예 설정 안 돼 있음~~ → **확인됨(2026-07-29,
   저장소 소유자): Render에 `DATA_REFRESH_TOKEN`이 설정돼 있다.** 이 시나리오는 배제된다
   — `/api/admin/refresh-data`가 항상 403인 상태는 아니라는 뜻(다만 오토로더 로컬
   `.env`의 값이 이 값과 정확히 일치하는지는 별개 확인 사항, 위 1번과 겹침).
4. 오토로더가 아닌 다른 경로로 `data` 브랜치가 바뀌는 경우(수동 편집, 장래의 다른
   자동화) — 애초에 이 트리거 호출 자체가 없으므로 2번만 믿어야 한다.

> **갱신(2026-07-29)**: `DATA_REFRESH_TOKEN`이 Render에 설정돼 있음을 저장소 소유자가
> 확인함에 따라 시나리오 3은 제외한다. 남은 1·2·4 시나리오(오토로더 로컬 env 미설정,
> 일시적 HTTP 실패, 오토로더 외 경로로 데이터 변경)가 라이브에서 실제로 발생했는지는
> 여전히 미확인이다("4번이 fail한 적이 없어서 잘 모르겠다" — 저장소 소유자, 최초 작성
> 시점 기준 발언). 토큰 설정 자체는 확인됐지만 **오토로더 쪽 `.env`가 항상 같은 값으로
> 유지되는지, 트리거 호출이 매번 성공하는지는 여전히 코드만으로는 검증 불가능한
> 운영 사실**이다.
