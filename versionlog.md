...
`git-rules.md`의 `hotfix/*` 브랜치 규칙 절 제목을 `hotfix/*` or `patch/`로 확장하고,
버그 수정이 아닌 단순 수정용 브랜치도 이 절의 규칙(항상 최신 `main`에서 생성, PR 최우선순위,
Patch 태깅)을 그대로 따르도록 설명을 보강했다. 코드 동작에는 영향 없음 — 문서 규칙 정정이라
Patch로 분류.
