...
흐름(곡별 fail-soft — 실패 곡은 스킵, 다음 실행에서 자동 재시도):
  ...
  ⑥ (선택) push 직후 배포 backend에 강제 리프레시 트리거 — `BACKEND_REFRESH_URL`(예:
     `https://<render-앱>.onrender.com/api/admin/refresh-data`)과 `DATA_REFRESH_TOKEN`
     (backend `render.yaml`의 동일 env와 값 일치 필요) 둘 다 설정돼 있을 때만 동작, 아니면
     조용히 스킵(fail-soft). backend가 웜 상태일 때 30분 주기 대기 없이 즉시 반영하기 위함
     (`_trigger_backend_refresh`).
...
def _trigger_backend_refresh() -> None:
    """push 직후 배포된 backend에 강제 리프레시 신호를 보낸다(fail-soft).

    백엔드가 잠들어 있으면 이 호출 자체가 콜드부팅을 깨우며 콜드부팅 중 어차피 최신 데이터를
    fetch하므로 무해하고, 이미 깨어있는(웜) 상태면 DATA_REFRESH_INTERVAL_SEC(기본 30분) 대기
    없이 즉시 반영된다. BACKEND_REFRESH_URL·DATA_REFRESH_TOKEN 둘 다 미설정이면(로컬/테스트
    ...
    try:
        ...
        print(f"백엔드 리프레시 트리거 완료: {resp.json()}")
    except Exception as exc:  # noqa: BLE001 — fail-soft, push는 이미 끝났음
