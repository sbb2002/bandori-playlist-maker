...
오리지널만 선곡, 커버·전체 요청만 명시적으로 해당 값을 따른다.

- 기준 커밋: `af1ad33`
- 관련 PR: [#90](https://github.com/sbb2002/bandori-playlist-maker/pull/90)
...
낸 콜라보/커버곡)이 필터·통계에서는 `various_artists`로 취급되면서도, 세트리스트
결과와 곡 추가 화면에는 실제 연주 유닛명이 표시되도록 `Song.display_band` 필드
추가. 트랙 사이 곡 추가(+) 버튼 아이콘이 너무 작다는 피드백에 따라 확대(24px→30px,
아이콘 13px→18px).

- 기준 커밋: `9938b92`
- 관련 PR: [#86](https://github.com/sbb2002/bandori-playlist-maker/pull/86)

## v2.6.1 — 2026-08-15

커스텀 모드 설정 UI(정서 지도·고급 설정 슬라이더)에 곡 희소 구간 시각 힌트 추가(빗금·추천
밴드). 선곡 로직 변경 없음.

- 기준 커밋: `fea7a5e`
- 관련 PR: [#84](https://github.com/sbb2002/bandori-playlist-maker/pull/84)

## v2.6.0 — 2026-08-15

선곡 로직을 4단계 깔때기로 재설계(질감 지표 희석 방지) + AI 모드 재생시간 미지정 시 품질
우선 유연화 + degraded 태그 노출 등 UI 버그 3건 수정.

- 기준 커밋: `07d3a85`
- 관련 PR: [#83](https://github.com/sbb2002/bandori-playlist-maker/pull/83)
...
5개 언어(KO/JA/EN/zh-Hans/zh-Hant) i18n 지원 추가 + AI 모드 요약 언어 미반영·대기시간
추정 부정확·라이트 모드 그래프 툴팁 가시성·오마카세 문구 모순 등 버그 수정.

- 기준 커밋: `76d75f7`
- 관련 PR: [#74](https://github.com/sbb2002/bandori-playlist-maker/pull/74)

## v2.4.1 — 2026-08-11

플레이바 라이트 테마 미적용, AI 모드 재생시간 하한 10→30분, 저장된 플리 스크롤바·정서맵
대비 등 테마 토글 사후 버그 수정 + 커스텀 모드 섹션 순서·구간조절 UI 재배치.

- 기준 커밋: `36ae722`
- 관련 PR: [#73](https://github.com/sbb2002/bandori-playlist-maker/pull/73)

## v2.4.0 — 2026-08-11

세트리스트 생성 중 레이스 컨디션 방지(프롬프트 입력창·커스텀 모드 파라미터 패널 잠금,
오마카세 조회 중 입력창 셔머 표시) + 생성 직후 실제 사용된 7종 지표를 커스텀 모드 UI에
즉시 반영 + 오마카세 버튼 5초 연타방지 쿨타임(시계방향 파이 애니메이션).

- 기준 커밋: `65c7dea`
- 관련 PR: [#71](https://github.com/sbb2002/bandori-playlist-maker/pull/71)

## v2.3.1 — 2026-08-11

밴드 별명 "레이"(RAISE A SUILEN)가 "플레이리스트"의 부분 문자열이라, 밴드 언급이 전혀
없는 프롬프트도 상시 오탐되던 버그 수정.

- 기준 커밋: `2f51e27`
- 관련 PR: [#70](https://github.com/sbb2002/bandori-playlist-maker/pull/70)

## v2.3.0 — 2026-08-11

Stage A 밝기 축을 mode_score/shape(형제 프로젝트 재활용 지표, 무드매칭 미검증)에서
에너지+신규 지표 6종 통합거리로 재설계 — 커스텀 모드 밝기 슬라이더가 선곡에 반영되지
않던 문제 해결.
