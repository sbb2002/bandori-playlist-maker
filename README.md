...
`v1.0.0`부터 버전 이력은 `main`이 아니라 `version` 브랜치의 `versionlog.md`에서 관리합니다
(`data`/`tools` 브랜치와 동일하게 PR 없이 직접 push하는 단일 상시 브랜치 — 매 머지마다 여기 로그를
추가하려고 `main`에 별도 PR을 여는 부담을 없애기 위함). 조회: `git show version:versionlog.md`,
또는 `git worktree add ../<임의경로> version`으로 펼쳐서 참고. 베타 기간 커밋별 변경 이력도 그
