...
곡 추가 미니 브라우저 검색이 장음(ー)이 든 일본어 제목을 한국식 관용 표기로 검색하지 못하던
문제 수정(예: `カーネーションの咲く日に`가 "카네이션"으로 검색 안 됨, `ホーミー・タイッ！！`가
"호미타이"로 검색 안 됨). v1.8.7에서 도입한 한글 음차(`song_hangul`)는 장음을 항상 직전 모음
반복으로만 풀어 원문식 표기("카아네에숀") 하나만 인덱싱했고, 이를 벗어나는 관용 표기는 외래어
사전에 곡마다 수동 등록해야 했다 — 오토로더로 자동 추가되는 신곡마다 구멍이 나는 구조적 한계였다
(이번에 실제로 걸린 게 방금 자동 반영된 두 신곡). `kana_to_hangul_variants()`를 신설해 장음
지점마다 {원문식 모음 반복, 생략, 한국식 이중모음(에이/오우)}을 자동 분기하고, ション/ジョン→
션/전 관용·어말 촉음 받침 생략·가운뎃점(・) 제거까지 함께 처리하도록 일반화했다(변형 최대 16개,
변형 0은 종전 `song_hangul` 출력과 동일해 하위호환 유지). 결과는 `Song.song_hangul_search`
필드(변형 " / " 병기, 검색 전용)에 캐싱해 `/api/songs` 응답과 프론트 검색 조건에 추가했다.
기존 표시 필드·오버라이드 사전은 그대로 두고 검색 매칭 범위만 넓힌 것이라 신규 기능(Minor)이
아니라 Patch로 분류.
