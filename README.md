...
`v1.0.0`부터 버전 이력은 `main`이 아니라 `version` 브랜치의 `README.md`에서 관리합니다
(`data`/`tools` 브랜치와 동일하게 PR 없이 직접 push하는 단일 상시 브랜치 — 매 머지마다 여기 로그를
추가하려고 `main`에 별도 PR을 여는 부담을 없애기 위함. 파일명은 2026-07-20부터 `versionlog.md`가
아니라 `README.md`다 — GitHub 브랜치 페이지에서 클릭 없이 바로 렌더링되도록). 조회:
